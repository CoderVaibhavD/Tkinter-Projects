...
def python_mode():
    ...
    messagebox.showinfo("Python Mode", "Save your work in the same directory as where the git bash terminal says. To run, type on the command line: Python whateveryourfilenameis.py. Where it says whateveryourfilenameis, put your own file name.")
...
def Makedir():
    ...
    s = Toplevel()
    l1 = Label(s, text ="Please enter your name for the personalised folder", font=font1)
    l1.pack(pady=20)

    myVar = StringVar()
    Thebox = Entry(s, textvariable=myVar)
    Thebox.pack(pady=20)

    def make():
        messagebox.showwarning("A note to self", "Please note that the directory will be in C:/Vaibhav/Coding/GUI_interface_with_python/Apps/Notepad ")
        makemydir(myVar.get())

    b1 = Button(s, text="Submit", height=5, width=20, command = make)
...
#-------------------------------------------------------------------------------
    ...
    file_menu.add_cascade(label="Logout", command=logout)
    file_menu.add_separator()
    file_menu.add_cascade(label = "New Window", command = new_win)
    file_menu.add_cascade(label = "Personalised folder", command = Makedir)
...
root.bind('<Control-Key-r>', on_closing)
#-------------------------------------------------------------------------------
